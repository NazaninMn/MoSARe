...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        metadata = ['Unnamed: 0', 'case_id', 'slide_id', 'age', 'site', 'survival_months', 'censorship', 'is_female', 'oncotree_code', 'train', 'cancer_label']
        ## Saurav edit
        metadata = [x for x in metadata if x in self.slide_data.columns]
        ## edit end
        ...
        if self.apply_sig:
            self.signatures = pd.read_csv('./datasets_csv/signatures.csv')
        else:
...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # label = torch.Tensor([self.slide_data['label'][idx]])
        label = torch.Tensor([self.slide_data['cancer_label'][idx]])
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    path_features = torch.load(casefeat_path,weights_only=True)
                    genomic_features = torch.tensor(self.genomic_features.iloc[idx])
                    return (path_features, genomic_features.unsqueeze(dim=0), label, event_time, c)
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, mode, 
        ...
        ## Saurav edit
        self.metadata = [x for x in self.metadata if x in self.slide_data.columns]
        ## edit end
