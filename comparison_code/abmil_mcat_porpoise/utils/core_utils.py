...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if os.path.exists(os.path.join(args.results_dir, "s_{}_minloss_checkpoint.pt".format(cur))):
        model.load_state_dict(torch.load(os.path.join(args.results_dir, "s_{}_minloss_checkpoint.pt".format(cur))))
    else:
        model.load_state_dict(torch.load(os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur))))

    if args.mode == 'coattn':
        results_val_dict, result_dict = summary_survival_coattn(model, val_loader, args.n_classes)
    else:
        results_val_dict, result_dict = summary_survival(model, val_loader, args.n_classes)

    print('Validation Results: {}'.format(result_dict))
    writer.close()
    return results_val_dict, result_dict


def train_loop_survival(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16, task_type='classification'):   
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        # print(data_WSI.shape, data_omic.shape)
        # print(data_omic)
        ...
        if (not isinstance(h, tuple)) and (task_type!='classification'):
            # print('Not Tuple')
            loss = loss_fn(h=h, y=y_disc, t=event_time, c=censor)
            loss_value = loss.item()
        elif (task_type=='classification'):
            # print(h, y_disc)
            loss = loss_fn(h,y_disc)
            loss_value = loss.item()
        else:
            # print('Tuple')
        ...
        label_list+=[y_disc.detach().cpu().item()]
        # print(h)
        preds_list+=[torch.argmax(torch.nn.functional.softmax(h, dim=1)).detach().cpu().item()]
        ...
        #     print('batch {}, loss: {:.4f}, label: {}, accuracy_score: {:.4f}, bag_size: {}'.format(batch_idx, loss_value, ds_string, accuracy_score(label_list, preds_list), data_WSI.size(0)))
...
def validate_survival(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None, task_type='classification'):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
...
def summary_survival(model, loader, n_classes):
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        label_list+=[y_disc.detach().cpu().item()]
        prediction=torch.argmax(torch.nn.functional.softmax(h, dim=1)).detach().cpu().item()
        ...
        # all_event_times[batch_idx] = event_time
        patient_results.update({slide_id: {'slide_id': np.array(slide_id), 'prediction': prediction, 'disc_label': y_disc.item()}})
